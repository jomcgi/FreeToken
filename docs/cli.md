...
| `--moe-cpu-decode-threads` | 0 (all workers) | Cap grouped decode participants because idle workers otherwise spin on SMT siblings of workers holding the layer's experts; prefill still uses the full pool. |
| `--moe-cpu-barrier {spin,hybrid}` | spin | Select pure spinning or spin-then-wait barriers; hybrid avoids leaving idle workers spinning on SMT siblings of workers holding the layer's experts. |
| `--moe-cpu-barrier-spin-us` | 30 | Microseconds hybrid barriers spin before waiting on a condition variable. |
| `--moe-cpu-precb {before,after}` | before | Order the DISK grouped-decode pre-run callback relative to notifying CPU workers. Try `after` for single-stream decode when `cpu_gil_us` plus `cpu_precb_us` is a large share of `cpu_wake_us`; the advisory WILLNEED work can then overlap CPU compute. In `after` mode, `cpu_precb_us` overlaps compute and is reported separately from wake. Prefill and host-function submissions retain `before` ordering. |
| `--moe-cpu-willneed {always,recent}` | always | Control decode-time WILLNEED advice for DISK experts. `always` preserves the existing path. `recent` skips experts touched on the same layer within `--moe-cpu-willneed-recent-steps` decode steps, default 256. If the rolling major-fault rate exceeds `--moe-cpu-willneed-fault-ceiling`, default 2000 faults per step, the executor restores `always` behavior for 256 steps. Prefill is unchanged. |
| `--moe-cpu-willneed-recent-steps` / `--moe-cpu-willneed-fault-ceiling` | 256 / 2000 | Recency window and 64-step rolling major-fault safety threshold used by `--moe-cpu-willneed recent`. Both must be positive. |
| `--moe-step-timing` | off | Append diagnostic decode timing fields. `cpu_wake_us`: average per-layer latency from submit entry to the first worker. In the default `before` mode, `cpu_groups_us`, `cpu_gil_us`, `cpu_precb_us`, and `cpu_notify_us` decompose and sum to `cpu_wake_us`. `cpu_coord_us`: coordinator overhead around submit/sync. `cpu_gpu_in_us`: per-layer time from the GPU doorbell issue to coordinator observation. `cpu_gpu_out_us`: per-layer time from the coordinator's done store to release of the GPU wait. The host-clock calibration stamp follows event synchronization, whose constant return-latency skew shifts `cpu_gpu_in_us` down and `cpu_gpu_out_us` up by the same amount. `cpu_d2h_us` and `cpu_h2d_us`: per-step GPU event totals for input copies and the output copy plus hot-partial add. `cpu_compute_us`: per-layer compute time. `cpu_signal_us`: per-layer signal time. `cpu_layers_per_step`: decode layer count. `cpu_expert_bytes_per_step`: expert weight bytes accessed. |
