...
class Engine:
    ...
    def _init_cpu_moe_executor(self, config: EngineConfig, cache, layers) -> None:
        ...
        executor = CpuMoeExecutor(
            ...
            moe_cpu_decode_threads=config.moe_cpu_decode_threads,
            ...
            moe_cpu_barrier=config.moe_cpu_barrier,
            moe_cpu_barrier_spin_us=config.moe_cpu_barrier_spin_us,
    ...
    def _drain_hot_adaptation_for_rebuild(cache) -> None:
        ...
        drain = getattr(cache, "drain_hot_adaptation_for_rebuild", None)
        ...
        *,
        ...
        if preserve_hot_state:
            if moe_cache_size is None or self.moe_offload_cache is None:
                raise CacheRebuildRejected(
                    "preserving HOT state requires an MoE cache resize"
                )
            protected = sum(self.moe_offload_cache.hot_expert_capacity.values())
            if not protected:
                raise CacheRebuildRejected(
...
_DENSE_MOE_SETTINGS = {
    ...
    "moe_cpu_threads": 0,
    "moe_cpu_decode_threads": 0,
    "moe_cpu_barrier": "spin",
    "moe_cpu_barrier_spin_us": 30,
